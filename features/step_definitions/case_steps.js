...
Given('there are {int} cases with a type of {string} with a status of {string} in the database', async function (number_of_cases, case_type, case_status) {
	this.case_type       = await this.db.one('select id, description, parent_id from case_type where description = ${case_type}', {case_type})
	this.case_status_type= await this.db.one('select id, description, parent_id from case_status_type where description=${case_status}', {case_status})
...
When('I add the communication event to the case', async function () {
	try {
		let result      =await this.client.mutate({
																								mutation : gql`mutation case_communication_event_add($case_id: ID!, $communication_event: InputCommunicationEvent!) {
																									case_communication_event_add(case_id: $case_id, communication_event: $communication_event) {
																										id
																								...
																								variables: {
																									...
																									communication_event: {
																										communication_event_status_type_id: this.communication_event.communication_event_status_type_id,
																										communication_event_type_id       : this.communication_event.communication_event_type_id,
																										contact_mechanism_type_id         : this.communication_event.contact_mechanism_type_id,
																										ended                             : this.communication_event.ended,
																										note                              : this.communication_event.note,
																										party_relationship_id             : this.communication_event.party_relationship_id,
																										started                           : moment(this.communication_event.started).format('YYYY-MM-DD')
																									}
																								}
																							})
		this.result.data=result.data.case_communication_event_add
	} catch (error) {
		this.result.error=error
	}
})
...
Then('the case contains the communication event', async function () {
	const updated_communication_event=await this.db.one('select id, started, ended, note, contact_mechanism_type_id, party_relationship_id, communication_event_status_type_id, communication_event_type_id, case_id from communication_event where case_id =${id}', this.case)
	expect(updated_communication_event.case_id).to.be.equal(this.case.id)
})
